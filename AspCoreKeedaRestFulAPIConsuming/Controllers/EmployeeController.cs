...
namespace AspCoreKeedaRestFulAPIConsuming.Controllers
{
    public class EmployeeController : Controller
    {
        ...
        public async Task<IActionResult> Create(Employee employee)
        {
            ...
            {
                BaseAddress = new Uri("http://localhost:57736/")
            };
            var response = await client.PostAsJsonAsync("api/employee", employee);//we don't know witch type of data we recieved so use var type variable here
        ...
        public async Task<IActionResult> Index()
        {
            ...
            {
                BaseAddress = new Uri("http://localhost:57736/")//here define your API App url here/Base Address <- by selecting Debug properties -> Url
            };
        ...
        public async Task<IActionResult> Edit(Employee employee)
        {
            ...
            var response = await client.PutAsJsonAsync($"api/employee/{employee.Id}", employee);//Here Same Code Copy Paste of Create Post Action Method and only changed HttpClient method here by PutAsJsonAsync
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
